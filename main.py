...
def retr_modl_trap(gdat, para):
    ...
    dura = para[2]
    dept = dilu * 0.6 
    numbphas = gdat.arrylcurbdtr.shape[0]
    indxphas = np.arange(numbphas)
    lcurmodl = np.ones(numbphas) + offs
    indxtimetran = tesstarg.util.retr_indxtimetran(gdat.arrylcurbdtr[:, 0], gdat.epocprio[0], gdat.periprio[0], dura)
...
def retr_llik_trap(gdat, para):
    ...
    llik = -0.5 * np.sum(((gdat.arrylcurbdtr[:, 1] - lcurmodl) / gdat.arrylcurbdtr[:, 2])**2)
...
def plot_pcur(gdat, strgpdfn):
    ...
    if strgpdfn == 'prio':
        ...
        arrypcur = gdat.arrypcurgdtr
        arrypcurbind = gdat.arrypcurgdtrbind
    ...
    for a in range(2):
        ...
        for j in gdat.indxplan:
            ...
            if strgpdfn == 'post':
                axis.plot(gdat.arrypcurmedimodlbind[j][:, 0], gdat.arrypcurmedimodlbind[j][:, 1], color='b')
            ...
            if strgpdfn == 'post':
                axis.plot(gdat.periprio[j] * 24. * gdat.arrypcurmedimodlbind[j][:, 0], gdat.arrypcurmedimodlbind[j][:, 1], color='b')
...
def main( \
    ...
    if gdat.booldetr:
        ...
        gdat.arrylcurbdtr[:, 1] = np.concatenate(lcurdetrregi)
    ...
    for j in gdat.indxplan:
        ...
        print('j')
        print(j)
        print('gdat.listindxtimeclen[j]')
        summgene(gdat.listindxtimeclen[j])
        print('')
    ...
    numbbins = 1000
    gdat.arrylcurbdtrbind = tesstarg.util.rebn_lcur(gdat.arrylcurbdtr, numbbins)
    ...
    for j in gdat.indxplan:
        gdat.arrypcurdetr[j] = tesstarg.util.fold_lcur(gdat.arrylcurbdtr, gdat.epocprio[j], gdat.periprio[j])
    ...
    if gdat.booldetr:
        ...
        for i in indxsplnregi:
            ## masked and non-detrended light curve
            ...
            indxtimetemp = indxtimeregi[i]
            axis[1].plot(gdat.arrylcur[indxtimetemp, 0] - gdat.timetess, lcurdetrregi[i], marker='o', ms=1, ls='', color='k')
        for a in range(2):
            axis[a].set_ylabel('Relative Flux')
    ...
    gdat.lcurgdtr = gdat.arrylcurbdtr[:, 1] - gdat.lcurbasealle
    gdat.arrylcurgdtr = np.copy(gdat.arrylcurbdtr)
    gdat.arrylcurgdtr[:, 1] = gdat.lcurgdtr
    
    gdat.arrypcurmedimodl = [[] for j in gdat.indxplan]
    gdat.arrypcurmedimodlbind = [[] for j in gdat.indxplan]
    gdat.arrypcurgdtr = [[] for j in gdat.indxplan]
    gdat.arrypcurgdtrbind = [[] for j in gdat.indxplan]
    for j in gdat.indxplan:
        ...
        gdat.arrypcurmedimodlbind[j] = tesstarg.util.rebn_lcur(gdat.arrypcurmedimodl[j], 2000)
        gdat.arrypcurgdtr[j] = tesstarg.util.fold_lcur(gdat.arrylcurgdtr[gdat.listindxtimeclen[j], :], gdat.epocprio[j], gdat.periprio[j])
        gdat.arrypcurgdtrbind[j] = tesstarg.util.rebn_lcur(gdat.arrypcurgdtr[j], numbbins)
    ...
    if gdat.boolphascurv:
        ...
        if gdat.pcurtype == 'sinu':
            ...
            for gdat.sinudetrtype in gdat.listsinudetrtype:
                ...
                for j in gdat.indxplan:
                    gdat.arrylcurcentdetr[j] = np.copy(gdat.arrylcurbdtr) 
                ...
                for gdat.pcursinetype in gdat.listpcursinetype:
                    ...
                    listalbgalle = 1e-6 * listdeptpday / (gdat.dicterrr['rrat'][0, 0] * \
                                                            gdat.dicterrr['rsma'][0, 0] / (1. + gdat.dicterrr['rrat'][0, 0]))**2
                    ...
                    gdat.numbtime = gdat.arrylcurbdtr.shape[0]
