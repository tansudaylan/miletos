...
def retr_dictmodl_mile(gdat, time, dictparainpt, strgmodl):
    ...
    if gdat.booldiag:
        if len(dictlistmodl['blin'][0][p]) == 0:
            print('')
            print('')
            print('')
...
def retr_dictderi_mile(para, gdat):
    ...
    for b in gdat.indxdatatser:
        for p in gdat.indxinst[b]:
            ...
            dictvarbderi['resi%s' % strg] = gdat.rflxthisfitt[b][p][:, gdat.fitt.listindxinstener[p]] - dictmodl['totl'][b][p]
...
def proc_alle(gdat, typemodl):
    ...
    if gdat.boolplottser:
        for b in gdat.indxdatatser:
            for p in gdat.indxinst[b]:
                for y in gdat.indxchun[b][p]:
                    plot_tser_mile(gdat, strgmodl, b, p, y, 'bdtr'+typemodl)
    ...
    if not os.path.exists(path):
        ...
        if boolchun:
            if boolflar:
                ...
                axis.plot(gdat.listarrytser['bdtr'][b][p][y][:, 0, 0] - gdat.timeoffs, \
                ...
                axis.fill_between(gdat.listarrytser['bdtr'][b][p][y][:, 0, 0] - gdat.timeoffs, \
...
def plot_modl(gdat, strgmodl, b, p, y, e, h):
    ...
    if gdat.typeinfe == 'samp':
        ...
        for w in range(gdat.numbsampplot):
            ...
            if gdat.fitt.typemodlenerfitt == 'full':
                dictmodl[namevarbsamp] = {'tser': gdat.dictsamp['modlfinetotl%s' % strg][w, :, e], 'time': timefine}
            else:
                dictmodl[namevarbsamp] = {'tser': gmod.listdictsamp[e]['modlfinetotl%s' % strg][w, :, 0], 'time': timefine}
        ...
        for namecompmodl in gdat.fitt.listnamecompmodl:
            ...
            for w in range(gdat.numbsampplot):
                ...
                if gdat.fitt.typemodlenerfitt == 'full':
                    dictmodl[namevarbsamp] = {'tser': gdat.dictsamp['modlfine%s%s' % (namecompmodl, strg)][w, :, e], 'time': gdat.timethisfittfine[b][p]}
                else:
                    dictmodl[namevarbsamp] = \
                                {'tser': gmod.listdictsamp[e]['modlfine%s%s' % (namecompmodl, strg)][w, :, 0], 'time': gdat.timethisfittfine[b][p]}
                if w == 0:
...
def setp_modlbase(gdat, strgmodl, h=None):
    ...
    if gmod.typemodl == 'psys' or gmod.typemodl == 'cosc' or gmod.typemodl == 'psysttvr' or gmod.typemodl == 'psyspcur':
        ...
        for j in gmod.indxcomp:
            ...
            # informed priors on the orbital period
            ...
            # prior on the mid-transit epoch
            if gdat.epocmtracompprio is not None:
                ## informed prior
                minmepocmtra = gdat.epocmtracompprio[j] - 0.2
                maxmepocmtra = gdat.epocmtracompprio[j] + 0.2
            else:
                ## uninformed prior
    ...
    for k in tqdm(range(len(listperi[i]))):
        ...
        for l in range(len(listdcyc[k])):
            ...
            for m in range(len(listepoc[k][l])):
                ...
                #print('srch_pbox_work()')
                #print('k, l, m')
                #print(k, l, m)

                indxitra = srch_pbox_work_loop(m, listphastemp, phasdiff, dydchalf)
    ...
    if boolproc:
        ...
        while True:
                ...
                if pathvisu is not None:
                    ...
                    if booldiag:
                        if not np.isfinite(rratcomp).all():
                            ...
                            print('')
                            print('')
                            print('dictpboxoutp[depttrancomp]')
                            print(dictpboxoutp['depttrancomp'])
                            print('rratcomp')
                            print(rratcomp)
                            print('listdept')
            ...
            if pathvisu is not None:
                ...
                axis.plot(listarrysrch[0][:, 0] - timeoffs, lcurpboxmetatemp, alpha=alphdata, marker='o', ms=1, ls='', color='gray')
                ...
                figr, axis = plt.subplots(figsize=figrsizeydobskin)
                axis.plot(dictpboxinte['phasdata'], dictpboxinte['rflxpserdata'], marker='o', ms=1, ls='', alpha=alphdata, color='gray')
...
def init( \
    ...
    if gdat.booltesskepl and gdat.boolinjeobsvanyy:
        numbtsec = len(gdat.listipnttcut)

        if gdat.boolutiltpxf:
            # determine for each sector whether a TFP is available
            booltpxf = retr_booltpxf(gdat.listipnttcut, gdat.listtsecspoc)
    ...
    if gdat.booltesskepl and gdat.boolinjeobsvanyy and len(gdat.listipntlygo) > 0:
    ...
    if gdat.booltesskepl:
        ...
        if gdat.boolinjeobsvanyy and len(gdat.listtsecspoc) > 0 and gdat.typelcurtpxftess == 'SPOC':
    ...
    if gdat.boolinfe and gdat.fitt.boolmodlpsys or gdat.boolsimurflx and gdat.true.boolmodlpsys:
        ...
        if not gdat.boolexecoffl and gdat.strgmast is not None and not gdat.boolsrchmastdone:
    ...
    if not 'raww' in gdat.listarrytser:
        ...
        for b in gdat.indxdatatser:
            for p in gdat.indxinst[b]:
                if (gdat.liststrginst[b][p].startswith('TESS_S') or gdat.liststrginst[b][p] == 'TESS'):
                    if len(gdat.listipnt) != len(gdat.listarrytser['raww'][b][p]):
                        print('')
                        print('')
                        print('')
                        print('b, p')
    ...
    if gdat.booldiag:
        for b in gdat.indxdatatser:
            for p in gdat.indxinst[b]:
                ...
                if gdat.liststrginst[b][p] == 'TESS' and not hasattr(gdat, 'listtsecspoc') and (gdat.liststrgtypedata[b][p] != 'simutargsynt'):
                    print('')
                    print('')
                    print('')
                    print('b, p')
                    ...
                    print('gdat.boolretrlcurmast[b][p]')
                    print(gdat.boolretrlcurmast[b][p])
                    raise Exception('listipnt is not defined while accessing TESS data of particular target.')
                ...
                if gdat.liststrginst[b][p] == 'TESS' and len(gdat.listipnt) != len(gdat.listarrytser['raww'][b][p]):
                    print('')
                    print('')
                    print('')
                    print('b, p')
    ...
    if gdat.boolsrchpbox:
        ...
        for p in gdat.indxinst[0]:
            ...
            gdat.dictpboxinpt['alphdata'] = gdat.alphdata
            
            gdat.dictpboxinpt['minmperi'] = 0.5
            gdat.dictpboxinpt['maxmperi'] = 30.
    ...
    if gdat.boolsrchflar:
        ...
        for p in gdat.indxinst[0]:
            for y in gdat.indxchun[0][p]:
                if gdat.boolplottser:
                    plot_tser_mile(gdat, strgmodl, 0, p, y, 'bdtr', boolflar=True)
            if gdat.boolplottser:
                plot_tser_mile(gdat, strgmodl, 0, p, None, 'bdtr', boolflar=True)
    ...
    #for b in gdat.indxdatatser:
    #    for p in gdat.indxinst[b]:
    #        gdat.arrytser['bdtrbind'][b][p] = rebn_tser(gdat.arrytser['bdtr'][b][p], delt=gdat.delttimebind)
    #        for y in gdat.indxchun[b][p]:
    #            gdat.listarrytser['bdtrbind'][b][p][y] = rebn_tser(gdat.listarrytser['bdtr'][b][p][y], delt=gdat.delttimebind)
    #            
